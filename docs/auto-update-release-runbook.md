...
The committed `src-tauri/tauri.conf.json` intentionally has updater artifact
generation disabled and no endpoint or public key. The workflow creates a
temporary tag-specific configuration after the public key is supplied. This
keeps the production feed gated until the Phase 5 rehearsal is approved.
...
5. Inspect the draft assets and `latest.json`: the manifest must contain the
   `windows-x86_64` entry, NSIS installer URL, release notes, UTC `pub_date`,
   and non-empty signature. Do not publish until the Windows rehearsal passes.
...
Do not switch the committed app configuration to the stable
`releases/latest/download/latest.json` endpoint until an older installed build
has completed the Phase 5 rehearsal. At that point, add the real public key and
approved endpoint through a reviewed configuration change, then repeat the
version, manifest, frontend, and native checks. A bad release is rolled forward
with a higher fixed version; do not rotate the updater key as the first response.
