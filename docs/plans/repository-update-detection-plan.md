...
- Add a distinct `detail-active` tier to the priority model for the repository whose details are currently open. While this tier is active, run an immediate targeted status refresh when details open, use working-tree filesystem events as refresh hints with debounce, emit a versioned repository-scoped `repository-changes-invalidated` event after successful refreshes, and expose a revision-aware changes read. The changes view should fetch on invalidation and keep a slower reconciliation safety net for missed events, watcher degradation, and sleep/wake recovery. Stop this working-tree monitoring when details close or the repository leaves the detail view.
